...
const parseAmountSatoshis = (amountBTC) => {
  if (typeof amountBTC !== "string" && typeof amountBTC !== "number") {
    throw new BitcoinTestnet4TransactionError("Invalid Bitcoin Testnet4 amount.");
  }
...
const validateTestnet4Address = (address) => {
  if (typeof address !== "string" || !address.trim()) {
    throw new BitcoinTestnet4TransactionError("Invalid Bitcoin Testnet4 destination address.");
  }

  try {
    const decoded = bitcoin.address.fromBech32(address.trim());
    if (decoded.prefix !== TESTNET4_NETWORK.bech32 || decoded.version !== 0 || decoded.data.length !== 20) {
      throw new Error("Unsupported address");
    }
    return bitcoin.address.toOutputScript(address.trim(), TESTNET4_NETWORK);
  } catch (error) {
    throw new BitcoinTestnet4TransactionError("Invalid Bitcoin Testnet4 destination address.");
  }
};
...
const getFeeRate = async () => {
  try {
    ...
    const providerFeeRate = Number(fees.halfHourFee || fees.hourFee || fees.fastestFee);
    if (Number.isFinite(providerFeeRate) && providerFeeRate > 0) {
      return Math.min(Math.max(Math.ceil(providerFeeRate), 1), MAX_FEE_RATE);
    }
  } catch (error) {
...
const estimateFee = (inputCount, outputCount, feeRate) => Math.ceil(
  (10 + (inputCount * 68) + (outputCount * 31)) * feeRate,
);

const buildSignedTransaction = ({ selectedUtxos, amountSatoshis, changeSatoshis, senderScript, destinationScript, keyPair }) => {
  ...
  psbt.addOutput({ script: destinationScript, value: amountSatoshis });
  if (changeSatoshis > DUST_THRESHOLD_SATOSHIS) {
    psbt.addOutput({ script: senderScript, value: changeSatoshis });
  }
  ...
  return {
    ...
    outputSatoshis: transaction.outs.reduce((total, output) => total + output.value, 0),
  };
};

const sendBitcoinTestnet4 = async ({ address, encryptedKey, toAddress, amountBTC }) => {
  ...
  if (typeof encryptedKey !== "string" || !encryptedKey) {
    throw new BitcoinTestnet4TransactionError("Bitcoin Testnet4 wallet is unavailable.");
  }

  const [utxos, feeRate] = await Promise.all([getAddressUtxos(address), getFeeRate()]);
  const confirmedUtxos = utxos
    .filter((utxo) => utxo.status?.confirmed && Number.isSafeInteger(Number(utxo.value)) && Number(utxo.value) > 0)
  ...
  for (const utxo of confirmedUtxos) {
    ...
    if (selectedSatoshis >= amountSatoshis + estimatedFee) break;
  }

  const estimatedFee = estimateFee(selectedUtxos.length, 2, feeRate);
  if (!selectedUtxos.length || selectedSatoshis < amountSatoshis + estimatedFee) {
  ...
  try {
    ...
    const keyPair = ECPair.fromWIF(privateKey, TESTNET4_NETWORK);
    const senderPayment = bitcoin.payments.p2wpkh({
      pubkey: Buffer.from(keyPair.publicKey),
      network: TESTNET4_NETWORK,
    });
    if (!senderPayment.address || senderPayment.address !== address || !senderPayment.output) {
      throw new BitcoinTestnet4TransactionError("Bitcoin Testnet4 wallet address does not match its signing key.");
    }

    const initialChange = selectedSatoshis - amountSatoshis - estimatedFee;
    const initial = buildSignedTransaction({
      selectedUtxos,
      amountSatoshis,
    ...
    const requiredFee = Math.ceil(initial.virtualSize * feeRate);
    const changeSatoshis = selectedSatoshis - amountSatoshis - requiredFee;
    const finalTransaction = changeSatoshis > DUST_THRESHOLD_SATOSHIS
    ...
    const feeSatoshis = selectedSatoshis - finalTransaction.outputSatoshis;
    const txid = await broadcastTransaction(finalTransaction.rawHex);

    return {
      success: true,
      message: "Bitcoin Testnet4 transaction broadcast successfully.",
      transaction: {
        txid,
        from: address,
        to: toAddress.trim(),
        amountBTC: amountSatoshis / SATOSHIS_PER_BTC,
        feeBTC: feeSatoshis / SATOSHIS_PER_BTC,
